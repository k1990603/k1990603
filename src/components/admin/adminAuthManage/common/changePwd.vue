<template>
  <div class="editPwdPage dialogTemplate">
    <!-- <div class="oldInfo">
      <el-input v-model="pwd" placeholder="请输入密码"></el-input>
      <el-input v-model="unPwd" placeholder="请输入确认密码"></el-input>
    </div> -->
    <div class="oldInfo">
      <div class="itemInfo">
        <label>账户名称</label>
    ...
    <el-form label-width="100px" :model="userInfo" status-icon :rules="userPwdRules" ref="userPwdInfo">
      ...
      <el-form-item>
        ...
        <el-button @click="$emit('changeRes')">取消</el-button>
      </el-form-item>
    </el-form>
    <!-- <span slot="footer" class="dialog-footer">
      <el-button type="primary" @click="submit">确 定</el-button>
      <el-button @click="$emit('changeRes')">取 消</el-button>
    </span> -->
  </div>
</template>
<script>
import { editUserPwd } from '@/api/admin/adminGroupManage'
      ...
      this.$refs['userPwdInfo'].validate(valid => {
        if (valid) {
          ...
          this.editUserPwd({
            ...
            new_user_password: this.userInfo.pwd
          }).then(res => {
            if (res.data.code != 0) {
              this.$message({
                type: 'error',

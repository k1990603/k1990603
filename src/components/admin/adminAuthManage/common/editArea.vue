...
import { getAdminDetail, editArea } from '@/api/admin/adminGroupManage'
export default {
  data() {
    return {
  ...
  methods: {
    ...
    getDefaultInfo() {
      this.getAdminDetail({ group_uuid: this.$route.params.uuid }).then(res => {
        if (res.data.code != 0) {
          this.$message({
            type: 'error',
            message: res.data.msg
          ...
          if (list.host_cluster_data_list) {
            ...
            this.hostpoolList.forEach(item => {
    ...
    onSubmit() {
      ...
      this.editArea(options).then(res => {
        if (res.data.code != 0) {
          this.$message({
            type: 'error',
            message: res.data.msg

        ...
        <div class="title">
            ...
            <div class="titLeft">
                <div class="titLeft-num">
                    <router-link to="/sf_cloud/workbench/adminWorkList">
            ...
            <div class="titRight" v-if="detailWorder.status=='处理中'&&questionList.length>0">
        ...
        <div class="main">
            <div class="main-left">
                        ...
                        <li class="detail-recode-item" v-for="(item,index) in questionList" :key="index">
                            ...
                            <div :class="[item.name == '顺丰云工程师:' ? 'selected' : '','detail-recode']">
                                <div class="detail-recode-title clearfix">
                                    ...
                                    <div class="pull-right detail-recode-time">{{item.created_at}}</div>
                                </div>
                ...
                <div class="leaveword">
                    ...
                    <div class="textarea">
                        <el-input type="textarea" v-model="detailWorder.message" :autosize="{ minRows: 6, maxRows: 8}" placeholder="请输入内容">
                        </el-input>
                    </div>
...
import { wkDetails,addWorkerOrder,adminConfirm } from '@/api/admin/adminWorkList'
export default {
  data() {
    return {
      fileList: [],
      ...
      questionList: [
        ...
        //   name: "问题描述",
        //   created_at: "2018-04-02 16:10:10",
        //   messageList: [
        //     { message: "机密信息：******" },
  ...
  methods: {
    ...
    adminConfirm,
    conrim() {
      this.adminConfirm({user_request_id:this.detailWorder.user_request_id}).then(res => {
        if (res.data.code == 0) {
          this.$message({
                        message: '提交成功',
                        type: 'success'
                      });
          this.$router.push({ path: '/sf_cloud/workbench/adminWorkList' })
          // this.getList()
        }
      })
    },
        ...
        if (res.data.code == 0) {
          ...
          for(let i=0,data=res.data.data;i<data.length;i++){
            // if(data[i].identity=="管理员"){
              ...
              data[i].name=data[i].identity+": "+data[i].user_id
    ...
    submitWorkerOrder(){
      // let data={
      //   user_request_id:this.detailWorder.user_request_id,
      //   user_id:this.detailWorder.user_id,
      //   identity:this.detailWorder.identity,
      //   message:this.detailWorder.message
      // };
    ...
    stepPeogress(){
      switch (this.detailWorder.status) {
        ...
        for(let i=0;i<this.questionList.length;i++){
          if(this.questionList[i].identity=='管理员'){
            this.detailWorder.status="处理中"
            this.stepPeogress()
          }
        }
        break;

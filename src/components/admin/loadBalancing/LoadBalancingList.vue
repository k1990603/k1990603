<template>
  <div class="load-balance-list">
      ...
      <el-table :data="tableData" :key='key' style="width: 100%" stripe>
        ...
        <el-table-column label="操作" width="150" fixed="right">
          <template slot-scope="scope">
            ...
            <span class="operateBtn" v-if='scope.row.state==0' @click="$router.push({path:`/sf_cloud/workbench/LBmonitor/${scope.row.uuid}`,query:{rowInfo:JSON.stringify(scope.row)}})">监听器</span>
          </template>
        </el-table-column>
      </el-table>
      <!-- 分页组件 -->
    ...
    <el-dialog title="创建负载均衡" @close='handleClose' class='dialogTemplate' :visible.sync="creatEdit" width="650px" center>
      <CreatLB @closeDialog="closeCreat" :tenantList='tenantList' :createNum='createNum'></CreatLB>
    </el-dialog>
    <!-- 删除负载均衡弹框 -->
    <el-dialog title="删除负载均衡" class='dialogTemplate' :visible.sync="deletelb" width="650px" center>
...
import selectComponent from './../../common/selectCommon'
import tableColumNameDialog from './../../common/tableColumName'
import { getLbList, deleteLb, changeName } from '@/api/admin/lb'
...
export default {
  ...
  methods: {
    ...
    toDetails(row) {
      this.$router.push({
        path: `/sf_cloud/workbench/LBdetails/${row.uuid}`,
    ...
    getSelect(item) {
      ...
      if (item.tenant_uuid === '-1') {
        ...
        this.getList({
          tenant: this.searchBox.tenant_uuid,
          project: 'all'
        })
      }
    },

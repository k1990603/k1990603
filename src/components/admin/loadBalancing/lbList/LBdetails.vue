    ...
    <div class='detailsContent'>
      ...
      <el-card class="box-card">
        ...
        <div class="item" v-if="info.network_type=='1'">
          ...
          <span class='item-value'>{{lbInfo.internet_vip }}</span>
        </div>
        <div class="item">
        ...
        <div class="item">
          <span class='item-label'>网段类型</span>
        ...
        <div class="item">
          <span class='item-label'>服务IP</span>
          <span class='item-value'>
            <span style='color:#999;' v-for='item in lbInfo.instance_info'>
              {{item.instance_ip}}
            </span>
          </span>
        </div>
...
export default {
  ...
  mounted() {
    this.lbInfo = JSON.parse(this.$route.query.info)
    this.getDetails()
  },
  methods: {
    changeName,

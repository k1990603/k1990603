<template>
  <div class="safeGroupTemplatePage">
    ...
    <div class="ruleTabs" v-loading="loading" element-loading-text="提交中,请耐心等待..." element-loading-spinner="el-icon-loading">
      ...
      <div class="ruleList">
        <div class="ruleBtnBox">
          ...
          <a>
            <el-button type="default" @click='downloadTmpTrigger' size="small" :disabled="isDrag || isEdit || isInsert || (multipleSelection.length>0)">下载模板</el-button>
          </a>
          <span style='display: inline-block;'>
            <el-upload :show-file-list='false' :headers="{'currentUserInfo':currentUserInfo}" :action="uploadInfo.importFileUrl" :data="uploadInfo.upLoadData" :onError="uploadError" :onSuccess="uploadSuccess" :beforeUpload="beforeAvatarUpload">
              <el-button type="default" size="small" :disabled="isDrag || isEdit || isInsert || (multipleSelection.length>0)">导入规则</el-button>
            </el-upload>
          </span>
          <a>
            <el-button type="default" @click='downloadRuleTrigger' size="small" :disabled="isDrag || isEdit || isInsert || (multipleSelection.length>0)">下载规则</el-button>
          </a>
...
export default {
  ...
  methods: {
    ...
    setUpLoadInfo() {
      ...
      console.log(this.uploadInfo.upLoadData)
    ...
    tabClick(tab, event) {
      ...
      this.setUpLoadInfo()
      this.downLoadRulesSrc = this.downLoadRules(
        `owner_uuid=${this.$route.params.uuid}&&rule_type=${this.ruleType}`
      )
      this.getList()
    },
    deleteTrigger() {

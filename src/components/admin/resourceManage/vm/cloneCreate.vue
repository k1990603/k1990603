<template>
  <div class="cloneCreateAdminPage">
    <div class="title">
      ...
      <div class="cloneVmInfo" v-loading="loading" element-loading-spinner="el-icon-loading" element-loading-text="提交中,请耐心等待...">
        <el-form label-position='left' label-width="120px" :model='newInfoForm' ref='cloneForm' :rules='newInfoRules'>
          ...
          <el-form-item label="配置" prop='flavor_id'>
            <el-table :data="formList.configList" style="width: 600px" height="315">
          ...
          <el-form-item label="应用管理员" prop='admin'>
            <el-select v-model="newInfoForm.admin" placeholder="请输入应用管理员">
              <el-option v-for="item in formList.projectMemberList" :key="item.userid" :label="item.username" :value="item.userid">
              </el-option>
            </el-select>
          </el-form-item>
...
export default {
  ...
  mounted() {
    ...
    this.getConfigInfo().then(res => {
      if (res.data.code == 0) {
        this.formList.configList = res.data.data
      }
    })
  },
  methods: {
    ...
    getDefaultInfo() {
      this.getVmDetails({
        ...
        if (res.data.code == 0) {
          ...
          this.defaultInfo.hc_name = this.defaultInfo.host_cluster_data_list[
            this.defaultInfo.host_cluster_data_list.length - 1
          ].name
          this.getProjectMember({
            project_uuid: this.defaultInfo.project_uuid
          }).then(res => {
            if (res.data.code == 0) {
              this.formList.projectMemberList = res.data.data
            }
          })
        }
      })
      ...
      this.$refs['cloneForm'].validate(valid => {
        if (valid) {
          ...
          let options = {
            ...
            flavor_id: this.formList.configList[this.newInfoForm.checkedConfig]
              .flavor_id,

<template>
  <div class="vmCreatePage">
    ...
    <el-form v-loading="loading" element-loading-spinner="el-icon-loading" element-loading-text="提交中,请耐心等待..." label-width="120px" :model="formData" :rules='createRules' ref='createForm'>
      <div class="selectBox" v-if='active == 1'>
        <el-form-item label="所属租户" prop="tenant_uuid">
          <el-select v-model="formData.tenant_uuid" @change='handltChangeTenant' placeholder="请选择">
            <el-option v-for="item in allListInfo.tenantList" :key="item.tenant_uuid" :label="item.tenant_name" :value="item.tenant_uuid">
            </el-option>
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="配置" prop='flavor_id'>
          <el-table :default-sort="{prop: 'flavor_cpu', order: 'ascending'}" :data="allListInfo.configList" style="width: 500px" height="400">
      ...
      <div class="selectBox" v-if='active == 3'>
        ...
        <el-form-item label="带宽" required v-if="formData.isPublic">
...
export default {
  ...
  mounted() {
    this.getUseTenant().then(res => {
      if (res.data.code == 0) {
        this.allListInfo.tenantList = res.data.data
      }
    })
  ...
  methods: {
    ...
    handleChangeSecurityType() {
      ...
      switch (this.formData.securityType) {
        case '0':
          this.$router.push('/sf_cloud/workbench/safeGroup')
          break
        case '1':
          this.getSecurityInfo({
            project_uuid: this.formData.project_uuid
    ...
    handltChangeTenant() {
      this.clearForm(
        ['projectList', 'areaList', 'dcList', 'hcList'],
        ['project_uuid', 'area_uuid', 'dc_uuid', 'hc_uuid']
      )
      this.allListInfo.tenantList.forEach(item => {
        if (item.tenant_uuid == this.formData.tenant_uuid) {
          this.allListInfo.projectList = item.project_info
          this.displayData.tenant_name = item.tenant_name
        }
      })
    },
    handltChangeProject() {

  ...
  <div class="editHostpool">
    ...
    <div>
      <div class="oldInfo">
        ...
        <div class="itemInfo">
          <label>项目数量</label>
          <span>{{defaultInfo.project_nums}}</span>
        </div>
        <el-form label-width="120px" status-icon>
          <el-form-item label="区域">
            <el-tree :props="props" :data="areaList" ref='tree' node-key="uuid" show-checkbox @check-change="handleCheckChange">
            </el-tree>
          </el-form-item>
          <el-form-item label="所属机房">
            <div class="selectBox">
              <el-table height="250" ref='datacenter' :data="datacenterList" tooltip-effect="dark" style="width: 500px" @selection-change="handleDatacenterChange">
          ...
          <el-form-item label="所属集群">
            <div class="selectBox">
              <el-table height="250" :data="hostpoolList" ref='hostpool' tooltip-effect="dark" style="width: 500px" @selection-change="handleHostpoolChange">
                <el-table-column type="selection" width="55">
                </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getDefaultLesseeInfo() {
      this.getLesseeInfo({ tenant_uuid: this.$route.params.uuid }).then(res => {
        if (res.data.code == 0) {
    ...
    handleHostpoolChange(val) {
      ...
      if (list.length <= 0) {
        ...
        list.forEach(item => {
          this.hostcluster_uuid_list.push(item.uuid)
        })
      }
    },
    cancel() {
    ...
    onSubmit() {
      ...
      let options = {
        tenant_uuid: this.$route.params.uuid,
        hostcluster_uuid_list: JSON.stringify(this.hostcluster_uuid_list)
      }
      this.editLesseeHostpoolInfo(options).then(res => {
        if (res.data.code != 0) {
          this.$message({
            type: 'error',
            message: res.data.msg

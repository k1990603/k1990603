    ...
    <div class="lessseeInfo">
      ...
      <el-form v-show="stepActive == 1" label-width="120px" :model="createInfo" status-icon :rules="createRules" ref="lesseeInfo">
        <el-form-item label="名称" prop='name'>
          <el-col :span="8">
            <el-input v-model="createInfo.name" placeholder="请输入项目名称"></el-input>
          </el-col>
        </el-form-item>
        <el-form-item label="所属租户">
          <el-select @change="getUsers" v-model="createInfo.tenant_uuid" filterable placeholder="请选择租户">
            <el-option v-for="item in lesseeList" :key="item.value" :label="item.tenant_name" :value="item.tenant_uuid">
            </el-option>
          </el-select>
          <router-link to="/sf_cloud/workbench/lesseeCreate">前往创建租户</router-link>
        </el-form-item>
        <el-form-item label="所有者">
          <el-select v-model="createInfo.ownerid" filterable placeholder="请输入所有者">
            <el-option v-for="item in userList" :key="item.value" :label="item.username" :value="item.userid">
            </el-option>
          </el-select>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    getUsers() {
      this.loadNode()
      this.createInfo.ownerid = null
      this.getLesseeMember({ tenant_uuid: this.createInfo.tenant_uuid }).then(
        res => {
          if (res.data.code != 0) {
            this.$message({
              type: 'error',
    ...
    // 上一步
    toNext(formName) {
      console.log(JSON.stringify(this.createInfo.hostcluster_uuid_list))
    ...
    onSubmit() {
      if (this.createInfo.hostcluster_uuid_list.length > 0) {
        ...
        this.addProject(options).then(res => {
          if (res.data.code == 0) {
            this.$message({
              type: 'success',
              message: '创建成功'
    ...
    handleHostpoolChange(val) {
      ...
      if (list.length <= 0) {
        ...
        list.forEach(item => {
          this.createInfo.hostcluster_uuid_list.push(item.hc_uuid)
        })
      }
      console.log(this.createInfo.hostcluster_uuid_list)
    },
    // 加载区域选择树结构
    loadNode() {
      this.getLesseeInfo({ tenant_uuid: this.createInfo.tenant_uuid }).then(
        res => {
          if (res.data.code == 0) {

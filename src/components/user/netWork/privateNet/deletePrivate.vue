  ...
  <div>
    <div class="deleteContent ">
      <div class="oldInfo">
        ...
        <div class="itemInfo">
          <label>所属区域</label>
          <span>{{deleteInfo.area_name}}</span>
        </div>
        <div class="itemInfo">
          <label>网络类型</label>
          <span>{{formateNetworkType(deleteInfo.network_type)}}</span>
        </div>
        <div class="itemInfo">
          <label>所属租户</label>
...
<script>
import { deleteNetWork } from '@/api/admin/netWork'
...
export default {
  ...
  methods: {
    ...
    deleteSubmit() {
      ...
      this.deleteNetWork(options).then(res => {
       if(res.data.code === 0){
         this.$message({
            type: 'success',
            message: '删除成功'

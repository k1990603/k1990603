<template>
  <div class="vmPage">
    <div class="defaultScope">
      ...
      <div class="searchList" v-if='requestList.projectList.length>0'>
        ...
        <ul class="childList" v-if="isShow">
          <li v-for="(item, index) in requestList.projectList" :key="index" @click="setSearchTitle(item)">
            {{item.project_name}}
          </li>
        </ul>
      </div>
      ...
      <el-row>
        <el-col :xs="24">
          <div class="batchBtn">
            ...
            <el-button type="success" :disabled="multipleSelection.length<=0" size="small" @click="someOperate({type:'startUp',info:multipleSelection})">开机</el-button>
            <el-button type="warning" :disabled="multipleSelection.length<=0" size="small" @click="someOperate({type:'shutDown',info:multipleSelection})">关机</el-button>
            <el-button type="warning" :disabled="multipleSelection.length<=0" size="small" @click="someOperate({type:'powerOff',info:multipleSelection})">强制关机</el-button>
            <el-button type="danger" :disabled="multipleSelection.length<=0" size="small" @click="someOperate({type:'reBoot',info:multipleSelection})">重启</el-button>
            <el-button type="danger" :disabled="multipleSelection.length<=0" size="small" @click="someOperate({type:'powerReBoot',info:multipleSelection})">强制重启</el-button>
            <el-button type="warning" :disabled="multipleSelection.length<=0" size="small" @click="someOperate({type:'delete',info:multipleSelection})">删除</el-button>

            <el-button type="info" :disabled="multipleSelection.length<=0" size="small" @click="someOperate({type:'reTry',info:multipleSelection})">重试</el-button>
            <!-- <el-button type="primary" :disabled="multipleSelection.length<=0" size="small">分配至项目</el-button> -->
          </div>
        </el-col>
      </el-row>
    ...
    <div class="vmList">
      <el-table :data="tableData" :key='key' style="width: 100%" stripe @selection-change="handleSelectionChange" v-loading='tableLoading'>
        ...
        <el-table-column prop="displayname" :show-overflow-tooltip='true' label="名称" min-width="120">
          <template slot-scope="scope">
            <span class='nameLink' @click='toDetails(scope.row)'>{{ scope.row.displayname }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="150" fixed="right">
          <template slot-scope="scope">
            <el-button type="text" :disabled="!(scope.row.status == '2'||scope.row.status == '3'||scope.row.status == '4'||scope.row.status == '7'||scope.row.status == '104')" @click='toConsole(scope.row)'>console</el-button>
              ...
              <el-dropdown-menu slot="dropdown" style='width:245px;'>
                <el-row>
                  <el-col :span="12">
                    ...
                    <el-dropdown-item :disabled="scope.row.status != 1 && scope.row.status != 3 && scope.row.status != 99 && scope.row.status != 100 && scope.row.status != 101 && scope.row.status != 102" :command="{type:'delete',info:[scope.row]}">删除</el-dropdown-item>
                    <el-dropdown-item :disabled="scope.row.status != 3" :command="{type:'diskExtent',info:scope.row}">磁盘扩容</el-dropdown-item>
                    <el-dropdown-item :disabled="scope.row.status != 3" :command="{type:'resetPassword',info:scope.row}">重置密码</el-dropdown-item>
                  </el-col>
                  <el-col :span="12">
                    <el-dropdown-item :disabled="scope.row.status != 3||scope.row.internet_ip_exist==0" :command="{type:'resetNet',info:scope.row}">调整网络</el-dropdown-item>
                    <el-dropdown-item :disabled="scope.row.status != 1" :command="{type:'resetConfig',info:scope.row}">调整配置</el-dropdown-item>
                    <el-dropdown-item :disabled="scope.row.status != 1 && scope.row.status != 3" :command="{type:'conferProject',info:scope.row}">分配至项目</el-dropdown-item>
                    <el-dropdown-item :disabled="scope.row.status != 1 && scope.row.status != 3" :command="{type:'cloneBackups',info:[scope.row]}">克隆备份</el-dropdown-item>
                    <el-dropdown-item :disabled="scope.row.status != 1" :command="{type:'cloneCreate',info:scope.row}">克隆创建</el-dropdown-item>
                    <el-dropdown-item :disabled="scope.row.status != 3" :command="{type:'netCardConfig',info:scope.row}">网卡配置</el-dropdown-item>
                    ...
                    <el-dropdown-item :disabled="scope.row.status != 1 && scope.row.status != 3" :command="{type:'seecurityConfig',info:scope.row}">配置安全组</el-dropdown-item>
    ...
    <el-dialog class="dialogTemplate" @close="handleClose" :title="title" width="650px" :visible.sync="vmOperateStatu.someOperate" center>
    ...
    <el-dialog title="重置密码" @close="handleClose" width="650px" class='dialogTemplate' :visible.sync="vmOperateStatu.resetPwd" center>
      <resetVmPwdDialog :rowInfo='rowInfo' :instanceUuid='instanceUuid' @resetPwdRes='resetPwdRes'></resetVmPwdDialog>
    </el-dialog>

    <el-dialog title="分配至项目" @close="handleClose" width="650px" class='dialogTemplate' :visible.sync="vmOperateStatu.editPro" center>
    ...
    <el-dialog title="调整配置" @close="handleClose" width="650px" class='dialogTemplate' :visible.sync="vmOperateStatu.config" center>
    ...
    <el-dialog title="调整网络" @close="handleClose" class="dialogTemplate" width="650px" :visible.sync="vmOperateStatu.internet" center>
      <editNet @editNetRes='editNetRes' :instanceUuid='instanceUuid' :rowInfo='rowInfo'></editNet>
    </el-dialog>
    <el-dialog class="dialogTemplate" @close="handleClose" title="网卡配置" width="650px" :visible.sync="vmOperateStatu.netCartConfigShow" center>
      <netCardConfig @diskScaleClick='netCartConfigClick' :instanceUuid='instanceUuid' :netCartId='netCartId' :rowInfo='rowInfo'></netCardConfig>
    </el-dialog>
...
import { downloadFile, formateVmStatus } from '@/filters/index'
import { getProjectList } from '@/api/user/userSecurityManage'
...
export default {
  ...
  methods: {
      ...
      this.getProjectList({
        ...
        if (res.data.code == 0) {
          if (res.data.data.length > 0) {
            ...
            this.requestList.projectList = [this.searchTitle].concat(
              res.data.data
            )
          } else {
            this.requestList.projectList = []
          }
        }
      })
    },
    ...
    toDetails(row) {
      this.$router.push(
        `/sf_cloud/workbench/userVmDetails/${row.instance_uuid}`
      )
    },
    ...
    changeSearch() {
      if (this.search.searchKey.length > 0) {
        ...
        if (this.searchBox.project_uuid === '-1') {
          this.getList(options)
        } else {
          options.project_uuid = this.searchBox.project_uuid
          this.getList(options)
        }
      } else {
    ...
    // vm创建
    createShow() {
      this.$router.push('/sf_cloud/workbench/userVmCreate')
    },
    ...
    someOperate(command) {
      ...
      switch (command.type) {
        ...
        case 'diskExtent':
          this.$router.push({
            path: `/sf_cloud/workbench/userVmDiskScale/${
        ...
        case 'cloneCreate':
          this.$router.push(
            `/sf_cloud/workbench/userCloneCreate/${command.info.instance_uuid}`
          )
          break
        case 'bindNet':
          this.title = '绑定公网IP'

<template>
  <div class="netCardUserPage" v-loading='loading'>
    <div>
      <div class="oldInfo">
        <div class="itemInfo">
          <label>名称</label>
              ...
              <div class="newInfo">
                <el-form :model='newIpInfo' ref='newIpInfoForm' label-width="120px" label-position='left'>
                  <el-form-item label='网卡IP类型'>
                    <el-select v-model="newIpInfo.newIpType" @change='handleChangeIpType' placeholder="请选择网卡IP类型">
...
export default {
  ...
  methods: {
    ...
    changeNetCardInitInfo,
    // 获取当前记录列表
    // 这里之所以写两个获取同样数据接口是因为.slice(0)数据深拷贝不起效果，我快疯掉了TT
    getList(uuid) {
      this.newInf = []

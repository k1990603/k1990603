  ...
  <div class="editHostpool">
    ...
    <div>
      <div class="oldInfo">
        ...
        <div class="itemInfo">
          <label>所有者</label>
          <span>{{defaultInfo.ownerid +'-'+defaultInfo.owner_name}}</span>
        </div>
        <div class="itemInfo">
          <label>所属租户</label>
        ...
        <el-form label-width="120px" status-icon>
          ...
          <el-form-item size="large">
            <el-button type="primary" @click='onSubmit'>确 定修改</el-button>
          </el-form-item>
        </el-form>
      </div>
    </div>
...
<script>
import { getTenantArea, getprojectInfo } from '@/api/user/userAuthManage'
import { editQuatoSubmit, getLesseeInfo } from '@/api/admin/lesseeManage'
export default {
  data() {
    return {
  ...
  methods: {
    ...
    getDefaultInfo() {
      this.getprojectInfo({ uuid: this.$route.params.uuid }).then(res => {
        if (res.data.code != 0) {
          this.$message({
            type: 'error',
